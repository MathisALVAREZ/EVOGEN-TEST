...
try:
from secret import GRAYLOG_URL_PROD, GRAYLOG_URL_RECETTE, GRAYLOG_USERNAME, GRAYLOG_PASSW
except ImportError:
raise SystemExit(
"Fichier 'secret.py' introuvable.\n"
"Créez-le avec GRAYLOG_URL_PROD, GRAYLOG_URL_RECETTE, GRAYLOG_USERNAME, GRAYLOG_PASSW
)
...
return [{"id": u["id"], "username": u.get("username",""), "full_name": u.get("full_na
...
try:
r = self.session.post(url, json={"selected_grantee_capabilities": grantees}, time
return (True, "OK") if r.status_code in (200,201,204) else (False, f"HTTP {r.stat
except Exception as e:
return False, str(e)
...
p = self._prepare(grn)
if p is None: return False, "prepare failed"
ex = {k:v for k,v in (p.get("selected_grantee_capabilities") or p.get("grantees") o
ok, msg = self._post_shares(grn, ex)
...
self.inner.bind("<Configure>", lambda e: canvas.configure(scrollregion=canvas.bbox("a
...
lightcolor=C["bg_white"], darkcolor=C["bg_white"], relief="flat", padding
...
self.btn_rec = self._env_btn(env_section, "RECETTE", C["recette"], C["recette_bg"],
...
fg="#6688AA", bg=C["sidebar"]).pack(anchor="w", pady=(0,8))
self._sb_action_btn(action_section, " Inspecter", C["blue_mid"], self._show_per
self._sb_action_btn(action_section, " Appliquer", C["green"], self._apply_pe
self._sb_action_btn(action_section, " Supprimer", C["own"], self._remove_p
tk.Frame(sb, bg="#243F6A", height=1).pack(fill="x", padx=16)
refresh_frame = tk.Frame(sb, bg=C["sidebar"], padx=16, pady=10)
refresh_frame.pack(fill="x")
self._sb_ghost_btn(refresh_frame, "↺ Rafraîchir les données", self._load_async).pack
# Statut en bas
sb.pack_propagate(False)
...
fg="#8BAAD4", bg=C["sidebar_dark"], wraplength=180, justify="left").pack(sid
...
tk.Frame(search, bg=C["border"], height=1).grid(row=1, column=0, columnspan=2, sticky
# Liste
...
# Séparateur vertical
tk.Frame(streams_area, bg=C["border"], width=1).grid(row=0, column=1, rowspan=3, stic
# Panel streams
...
self._small_link_btn(cat_btns, "Tout ✓", self._sel_all_cats).pack(side="left", self._small_link_btn(cat_btns, "Tout ✗", self._desel_cats).pack(side="left")
padx=8
...
tk.Frame(sf, bg=C["border"], height=1).grid(row=1, column=0, columnspan=2, sticky="ew
# Liste
...
# Footer
tk.Frame(panel, bg=C["border"], height=1).grid(row=2, column=0, columnspan=2, sticky=
footer = tk.Frame(panel, bg=C["bg_panel"], pady=6, padx=12)
footer.grid(row=3, column=0, columnspan=2, sticky="ew")
self._small_link_btn(footer, "Tout sélectionner", self._sel_all_streams).pack(side
tk.Label(footer, text=" · ", fg=C["text3"], bg=C["bg_panel"], font=FONT_SMALL).pack(s
self._small_link_btn(footer, "Tout désélectionner", self._desel_streams).pack(side="
...
# Tags log
self.log_box.tag_config("ok", foreground=C["success"], font=("Consolas", 9, self.log_box.tag_config("err", foreground=C["error"], font=("Consolas", 9, self.log_box.tag_config("warn", foreground=C["warning"], font=("Consolas", 9, self.log_box.tag_config("info", foreground=C["blue_mid"], font=("Consolas", 9))
self.log_box.tag_config("skip", foreground=C["text3"], font=("Consolas", 9))
self.log_box.tag_config("head", foreground=C["blue"], font=("Consolas", 9, self.log_box.tag_config("ts", foreground=C["text3"], font=("Consolas", 9))
"bold"
"bold"
"bold"
"bold"
# ── Widgets helpers ───────────────────────────────────────────────────────
None:
def _section_header(self, parent, title: str, subtitle: str = "", row: int = 0) -> f = tk.Frame(parent, bg=C["bg_white"])
f.grid(row=row, column=0, columnspan=10, sticky="ew")
inner = tk.Frame(f, bg=C["bg_white"], padx=16, pady=14)
inner.pack(fill="x")
tk.Label(inner, text=title, font=FONT_HEADER, fg=C["text"], bg=C["bg_white"]).pack(si
if subtitle:
...
self.env = env
self.client = GraylogClient(GRAYLOG_URL_PROD if env == "PROD" else GRAYLOG_URL_RECETT
...
self._on_stream_sel()
self._log(f" Catégorie(s) {sorted(cats)} → {len(self.stream_lb.curselection())} stre
...
if n:
names = [self.user_lb.get(i).split("(")[0].strip() for i in self.user_lb.curselec
...
threading.Thread(target=lambda u=uid: self.perms_cache.update({u: self.client
daemon=True).start()
...
"Veuillez sélectionner au moins un utilisateur.", parent=s
return False
...
self.after(0, self._log, f" ⊘ [{uname}] {title}", "skip"); skip += 1; c
s, msg = self.client.set_permission(sid, uid, cap)
if s:
self.after(0, self._log, f" ✓ [{uname}] {title}", "ok"); ok += 1
else:
self.after(0, self._log, f" ✗ [{uname}] {title} — {msg}", "err"); err
...
self.after(0, (messagebox.showinfo if not err else messagebox.showwarning), "Résultat
...
self.after(0, self._log, f" ✗ [{uname}] {title} — {msg}", "err"); err
if ok: self.perms_cache[uid] = self.client.get_user_permissions(uid)
self.after(0, self._log, f" → {ok} supprimées · {err} erreurs", "ok" if not err el
# ── Logs ──────────────────────────────────────────────────────────────────
