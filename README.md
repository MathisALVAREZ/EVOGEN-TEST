...
class CopyPermsWindow(tk.Toplevel):
    ...
    def _build(self) -> None:
        ...
        src_cb = ttk.Combobox(body, textvariable=self.src_var,
                               values=unames, state="readonly", font=FONT_LABEL)
        src_cb.grid(row=1, column=0, sticky="ew", pady=(0,16))
...
class CompareWindow(tk.Toplevel):
    ...
    def _build(self) -> None:
        ...
        ttk.Combobox(top, textvariable=self.ua_var, values=unames,
        ...
        ttk.Combobox(top, textvariable=self.ub_var, values=unames,
                     state="readonly", font=FONT_LABEL
    ...
    def _run(self) -> None:
        if not self.ua_var.get() or not self.ub_var.get():
            ...
            return
        ia = self.ua_var.current(); ib = self.ub_var.current()
